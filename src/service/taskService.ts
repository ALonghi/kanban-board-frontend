...
export default class TaskService {
    static async createTask(req: CreateTaskRequest): Promise<ITask> {
        try {
            Logger.info(`Adding task ${JSON.stringify(req)} to board ${req.board_id}`);
            ...
            Logger.error(
                `Error in creating task (${JSON.stringify(req)}) for board ${req.board_id}: ${e.message || e}`
            );
    ...
    static async updateTask(task: ITask, boardId: IBoard['id']): Promise<ITask> {
        try {
            Logger.info(`Updating task ${task.id} for board ${boardId}  with ${JSON.stringify(task)}`);
            const response = (await axios
            ...
            Logger.error(
                `Error in updating task for board ${boardId} (${task.id}) (${JSON.stringify(
                    task
                )}: ${e.message || e}`
            );
            return Promise.reject(e);
    ...
    static async deleteTask(taskId: ITask['id'], boardId: IBoard['id']): Promise<void> {
        try {
            ...
            Logger.error(
                `Error in deleting task ${taskId} from board ${boardId}: ${e.message || e}`
            );
            return Promise.reject(e);
        }
    }

    static async updateTasks(tasks: Omit<ITask, "position">[], boardId: IBoard['id']): Promise<ITask[]> {
        const tasksIds = tasks?.map(t => t.id) || []
        try {
            Logger.info(`Updating ${tasksIds.length} tasks ${tasksIds} for board ${boardId}  with ${JSON.stringify(tasks)}`);
            const response = (await axios
                .put(`/tasks`, tasks)
                .then((res) => res.data)) as ApiResponse<ITask[]>;
            if (!response.success || !response.data)
