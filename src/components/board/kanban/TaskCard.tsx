...
export default function TaskCard({
    ...
    const [showComponent, setShowComponent] = useState<boolean>(!!task?.id || true)
    ...
    const handleDeleteClick = async () => {
        const promise = task?.id ? onDelete(task.id) : Promise.resolve(setShowComponent(false))
        await promise.then(() => {
            setIsFocused(false)
            setTaskTitle("")
        })
              ...
              {isNew
                  ...
                  <p
                      className={`mr-auto ml-0 overflow-x-none
                        font-normal text-sm p-0 h-fit max-h-full ${isFocused ? `w-8/12` : `w-max`}`}
                  >{task.title}</p>
              }
